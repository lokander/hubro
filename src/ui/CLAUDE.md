...
- A signal read/written from a root `spawn_forever` task must be `Signal::new_in_scope(.., ScopeId::ROOT)`; a component-scoped one trips a `__copy_value_hoisted` runtime warning and can fail after that scope drops. **For `AppState` this is unconditional — every field is root-scoped, enforced by `every_app_state_signal_is_root_scoped`.** Deciding it per field was tried and did not hold: reachability travels through method calls, so nothing at a task's call site shows which signals it ends up touching (18 of 27 fields were reachable, twelve of them only through a method call; three had been noticed — FRE-156). No *behavioural* test can catch a regression here, because the warning exists only while the app is running — which is why that guard reads the constructor out of the source instead.

The SQL editor's CodeMirror bundle (`assets/codemirror.js`) is **committed and rebuilt by hand**: editing `assets/codemirror-entry.js` changes nothing that ships until you run the esbuild command in `assets/codemirror-README.md`. `the_committed_bundle_is_not_a_pre_fre_154_build` only catches a skipped rebuild for the code it names.
