...
/// Maps SQL Server's auto-assignment flavors onto [`Generated`]:
///
/// - computed columns are always database-assigned and never writable →
///   [`Generated::Always`] (like Postgres `GENERATED ALWAYS AS (…) STORED`);
...
///   user-writable → [`Generated::Always`];
/// - IDENTITY maps to [`Generated::ByDefault`] like a by-default identity
///   sequence: the database assigns the value, so the column is never
///   required on insert (`SET IDENTITY_INSERT` is the explicit override).
fn mssql_generated(is_identity: bool, is_computed: bool, type_base: &str) -> Generated {
    if is_computed || type_base == "timestamp" || type_base == "rowversion" {
        Generated::Always
    } else if is_identity {
        Generated::ByDefault
    } else {
...
mod tests {
    ...
    fn generated_maps_identity_computed_and_rowversion() {
        ...
        // IDENTITY is auto-assigned but overridable via IDENTITY_INSERT.
        assert_eq!(mssql_generated(true, false, "int"), Generated::ByDefault);
