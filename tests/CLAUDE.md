...
**Postgres and SQL Server suites get a database per test binary** (FRE-127). `common::pg_test_url()` / `common::mssql_test_url()` read the URL from the environment, create `hubro_test_<pid>_<unix_secs>` on that server, and hand back the URL naming it — so each suite's fixtures are its own even though cargo runs the binaries in parallel. Use them: reading `HUBRO_PG_TEST_URL`/`HUBRO_MSSQL_TEST_URL` directly puts a suite back in the shared database alongside everyone else's identically-named fixtures. Measured before the change, two concurrent copies of `db_postgres` failed 4–13 tests each and `db_sqlserver` 4–9 — mostly `relation … does not exist` and "Invalid object name", with SQL Server's deadlock-victim kill (1205) as the tail.

Those databases are **left behind at exit** (a test binary has no teardown hook) and swept by a later run once they are ten minutes old — the age is what makes the sweep safe against a concurrently-starting binary, whose database is seconds old. Expect a handful on a busy container and none after a quiet ten minutes. The single-suite engines (Timescale, Citus, CockroachDB, …) still share one database; nothing runs concurrently against them today.
