...
ready() {
  local engine=$1 set=$2 name attempt
  name=$(name_for "$engine" "$set")

  for attempt in $(seq 1 "${TEST_DB_TIMEOUT:-120}"); do
...
probe() {
  ...
  case "$engine" in
    ...
    materialize | risingwave)
      # Neither image ships a client, so probe the port from the host.
      local port
      port=$(docker port "$name" | head -1 | sed 's/.*://')
...
engines_from_args() {
  if [ "$#" -eq 0 ]; then
    ...
    local engine
    for engine in "$@"; do
      port_base "$engine" >/dev/null # validates
      echo "$engine"
    done
  fi
}
...
cmd_up() {
  ...
  shift
  local engines
  mapfile -t engines < <(engines_from_args "$@")
  echo "starting set $set: ${engines[*]}"
  local engine
  for engine in "${engines[@]}"; do create "$engine" "$set"; done
  # Waited for in a second pass so every container's boot overlaps.
  for engine in "${engines[@]}"; do ready "$engine" "$set"; done
  echo
