...
Nothing in the suite covers rendering, so a UI change wants the app driven: FRE-148's model tests all passed while the screen stated the reason twice and still offered Export on an unbrowsable object. Note the app writes to the *real* `$XDG_CONFIG_HOME/hubro/` — a connection added while testing persists in `connections.toml` and `session.toml`, so remove what you added.
