...
- Postgres integration tests skip unless `HUBRO_PG_TEST_URL` is set; SQL Server tests skip unless `HUBRO_MSSQL_TEST_URL` is set; TimescaleDB tests skip unless `HUBRO_TIMESCALE_TEST_URL` is set; Citus tests skip unless `HUBRO_CITUS_TEST_URL` is set; SSH-tunnel tests need `HUBRO_SSH_TEST` (+ `HUBRO_SSH_TEST_KEY`/`_ENC_KEY`). Point them at the Docker `hubro-pg-test` (host port 5433) / `hubro-mssql-test` (14333) / `hubro-timescale-test` (5434) / `hubro-citus-test` (5435) / `hubro-ssh-test` (2222) containers; the exact `docker run` commands live in the test-file headers (`tests/db_postgres.rs`, `tests/db_sqlserver.rs`, `tests/db_timescale.rs`, `tests/db_citus.rs`, `tests/tunnel.rs`). The Citus URL needs `?sslmode=disable` — that image ships an X.509 v1 certificate rustls won't parse (FRE-89).
